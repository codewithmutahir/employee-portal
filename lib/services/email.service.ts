/**
 * Email service – Mailtrap and all templated emails.
 * Used by Server Actions and (later) API routes.
...
export async function sendEmail(options: EmailOptions): Promise<EmailResult> {
  ...
  try {
    ...
    const data = (await response.json()) as { message_ids?: string[]; id?: string };
    return {
      success: true,
